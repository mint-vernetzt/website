...
import { getNewsSwiperItemProps } from "./utils";
...
test("render headline and body", () => {
  const props: NewsSwiperProps = {
    headline,
    body,
    linkToOverview,
  };

  render(<NewsSwiper {...props} />);
...
test("add news Swiper items", () => {
  const newsSwiperItemProps1 = getNewsSwiperItemProps();
  const newsSwiperItemProps2 = getNewsSwiperItemProps();
  const newsSwiperItemProps3 = getNewsSwiperItemProps();
  //const forcedNumberOfTags = 5;
  const newsSwiperItemProps4 = getNewsSwiperItemProps({
    //numberOfTags: forcedNumberOfTags,
  });
  const newsSwiperItemProps5 = getNewsSwiperItemProps({ withImage: true });
  ...
  const props = {
    ...
    newsSwiperItemsProps: [
      newsSwiperItemProps1,
      newsSwiperItemProps2,
      newsSwiperItemProps3,
      newsSwiperItemProps4,
      newsSwiperItemProps5,
    ],
  };
  ...
  const newsSwiperItemDom = allListItems.filter((listitem) => {
