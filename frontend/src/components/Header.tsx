...
function SubMenuItem(props: { menu: MenuItem }) {
  const { title, description, url, image, svg } = props.menu;
  ...
  return (
    ...
    >
      <div className="hidden w-24 mr-6 shrink-0 rounded-l-md">
        ...
        {!svg && image && (
          <GatsbyImage
            image={image}
            alt={title}
            className="w-full h-full rounded-l-md cover"
          />
        )}
      </div>
      <div className="md:py-2 md:px-4">
        ...
        <p className="md:pb-2 lg:text-base">{description}</p>
      </div>
    </Link>
  );
}
  ...
  return (
    <div className={`header-section shadow-lg`}>
      <button
        ...
        className={`w-screen h-screen absolute top-0 left-0 z-20 hidden ${
          activeMenu !== null ? "lg:block" : "hidden"
        }`}
      ></button>
                    ...
                    <>
                      <a
                        className={`flex items-center font-bold lg:inline-block py-1 lg:px-4 lg:rounded-lg lg:hover:bg-blue-500 lg:hover:text-neutral-200 ${
                          activeMenu === index
                            ? "lg:bg-blue-500 lg:text-neutral-200"
                            : ""
                        }`}
                        href="#"
                        onClick={(e) => {
                      ...
                      >
                        ...
                        <button
                          className={`ml-auto lg:hidden items-center h-5 w-5 flex item-center justify-center`}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
                            ...
                            <div className="lg:w-1/2 bg-white lg:rounded-bl-2xl">
                              <ul className="menu pt-4 lg:p-8">
                                {mainMenu.items.map((menu) => (
                                  <li
                                    key={mainMenu.url + menu.url}
                                    className="mb-6 lg:last:mb-0"
                                  >
