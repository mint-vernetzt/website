...
function SubMenuItem(props: { menu: MenuItem }) {
  ...
  return (
    <Link to={url} className="flex items-stretch text-left lg:hover:bg-[#F4F5F6] lg:rounded-lg">
      <div className="hidden md:block w-24 mr-6 shrink-0 rounded-l-md">
        {image && <GatsbyImage image={image} alt={title} className="w-full h-full rounded-l-md cover" />}
      </div>
...
function Teaser(props: { menu: MenuItem }) {
  ...
  return (
    <Link to={url} className="flex items-stretch text-left hover:bg-white/50 rounded-lg">
      <div className="hidden md:block w-24 mr-6 shrink-0 rounded-l-md">
        {image && <GatsbyImage image={image} alt={title} className="w-full h-full rounded-l-md cover" />}
      </div>
      <div className="md:p-2">
                            ...
                            <div className="lg:w-1/2 bg-white lg:rounded-bl-2xl">
                              <ul className="menu pt-4 lg:p-8">
                                {mainMenu.items.map((menu) => (
                                  <li key={mainMenu.url + menu.url} className="mb-6 lg:last:mb-0">
                            ...
                            <div className="hidden lg:block w-1/2 bg-[#EFE8E6] rounded-br-2xl">
                              <ul className="teaser p-8">
                                {mainMenu.teaser.map((teaser) => (
                                  <li key={mainMenu.url + teaser.url} className="mb-6 last:mb-0">
                                    <Teaser menu={teaser} />
                                  </li>
                                ))}
                              </ul>
